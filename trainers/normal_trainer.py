...
class NormalTrainer(object):
    ...
    def get_model_params(self):
        return self.model.to('cpu').to_static_state_dict()
...
#  `mode` choices: ['MODEL', 'GRAD', 'MODEL+GRAD']
    ...
    def train_mix_dataloader(self, epoch, trainloader, device, **kwargs):
        ...
        logging.info('\n=> Training Epoch #%d, LR=%.4f' % (epoch, self.optimizer.get_lr()))
        for batch_idx, (x1, x2,x3, y1, y2,y3) in enumerate(trainloader):
            ...
            x = paddle.concat((x1, x2,x3))
            y = paddle.concat((y1,y2,y3))
            ...
            if self.args.fedprox:
                ...
                for name, param in self.model.named_parameters():
                    fed_prox_reg += ((self.args.fedprox_mu / 2) * \
                        paddle.norm((param - previous_model[name].data.to(device)))**2)
                loss += fed_prox_reg
            # ========================FedProx=====================#
