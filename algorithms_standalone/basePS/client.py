...
# -------------------------VAE optimization tool for different client------------------------#
        self.vae_optimizer =  AdamW(
            parameters=self.vae_model.parameters(), 
            learning_rate=1.e-3, beta1=0.9, beta2=0.999, weight_decay=1.e-6)
    ...
    def _attack(self,size, mean, std):  #
        rand = paddle.normal(mean=mean, std=std, size=size).to(self.device)
        return rand
    ...
    def aug_classifier_train(self, round, epoch, optimizer, aug_trainloader):
        ...
        for batch_idx, (x, y) in enumerate(aug_trainloader):
            ...
            x, y, y_b = x.to(self.device), y.to(self.device).reshape([-1,]), y_b.to(self.device).reshape([-1, ])
    ...
    def mosaic(self, batch_data):
        ...
        if self.args.dataset =='fmnist':
            ...
            c, w, h = 3, 32, 32
        aug_data = paddle.zeros((self.args.VAE_aug_batch_size, c, w, h))
        ...
        for k in range(self.args.VAE_aug_batch_size):
            ...
            img4 = paddle.zeros(batch_data[0].shape)
    ...
    def aug_VAE_train(self, round, epoch, optimizer, aug_trainloader):
        self.vae_model.train()
        self.vae_model.training = True
        self.vae_model.stop_grad = False
        ...
        for batch_idx, (x, y) in enumerate(aug_trainloader):
            ...
            aug_l3 = -0.5 * paddle.sum(1 + aug_logvar - aug_mu.pow(2) - aug_logvar.exp())
    ...
    def train_whole_process(self, round, epoch, optimizer, trainloader):
        ...
        logging.info('\n=> Training Epoch #%d, LR=%.4f' % (epoch, optimizer.get_lr()))
        ...
        for batch_idx, (x, y) in enumerate(trainloader):
            ...
            cross_entropy = F.cross_entropy(out[: batch_size * 2], paddle.repeat_interleave(y,2))
            x_ce_loss = F.cross_entropy(out[batch_size * 2:], y)
            l1 = F.mse_loss(gx, x)
            l2 = cross_entropy
            l3 = -0.5 * paddle.sum(1 + logvar - mu.pow(2) - logvar.exp())
    ...
    def train_vae_model(self,round):
        ...
        train_dataloader = data.DataLoader(dataset=train_dataset, batch_size=32, shuffle=True,
                                                       drop_last=False)
        aug_vae_dataloader = data.DataLoader(dataset=aug_vae_dataset, batch_size=32, shuffle=True,
                                                         drop_last=False)
    ...
    def generate_data_by_vae(self):
        ...
        generate_dataset = Dataset_Personalize(ori_data, targets, transform=generate_transform)
        generate_dataloader = data.DataLoader(dataset=generate_dataset, batch_size=self.args.VAE_batch_size,
        ...
        with paddle.no_grad():
            for batch_idx, (x, y) in enumerate(generate_dataloader):
                ...
                if batch_idx == 0:
                    ...
                    self.local_share_data1 = paddle.concat((self.local_share_data1, rx_noise1))
                    self.local_share_data2 = paddle.concat((self.local_share_data2, rx_noise2))
                    self.local_share_data_y = paddle.concat((self.local_share_data_y, y))
    ...
    def sample_iid_data_from_share_dataset(self,share_data1,share_data2, share_y, share_data_mode = 1):
        ...
        for label in range(self.args.num_classes):
            ...
            if label == 0:
                ...
                if share_data_mode == 1:
                    epoch_data = paddle.concat((epoch_data, share_data1[sample]))
                elif share_data_mode ==2:
                    epoch_data = paddle.concat((epoch_data, share_data2[sample]))
                epoch_label = paddle.concat((epoch_label, share_y[sample]))

        last_sample =  random.sample(range(self.dataset_num), last) 
        if share_data_mode == 1:
            epoch_data = paddle.concat((epoch_data, share_data1[last_sample]))
        elif share_data_mode == 2:
            epoch_data = paddle.concat((epoch_data, share_data2[last_sample]))
        epoch_label = paddle.concat((epoch_label, share_y[last_sample]))
    ...
    def construct_mix_dataloader(self, share_data1, share_data2, share_y, round):
        ...
        self.local_train_mixed_dataloader = data.DataLoader(dataset=train_dataset,
                                                                  batch_size=32, shuffle=True,
                                                                  drop_last=False)
    ...
    def get_vae_para(self):
        return deepcopy(self.vae_model.to('cpu').to_static_state_dict())
