...
__all__ = ['setup_run', 'Logger',  'setup_logger', 'set_random_seed', 'accuracy', 'AverageMeter',  'mixup_data', 'total_correlation',
...
#                         state['max_exp_avg_sq'] = paddle.zeros_like(p.data)
...
#                 p.data.add_(-step_size, paddle.mul(p.data, group['weight_decay']).addcdiv_(1, exp_avg, denom))

#         return loss

class AverageMeter(object):
    """Computes and stores the average and current value
       Imported from https://github.com/pypaddle/examples/blob/master/imagenet/main.py#L247-L262
    """
...
def mixup_data(x, y, alpha=1.0, use_cuda=False):
    ...
    if use_cuda:
        index = paddle.randperm(batch_size).cuda()   # 随机生成batch_size的随机排
    else:
...
def gaussian_log_density(samples, mean, log_var):
    pi = paddle.tensor(math.pi)
    normalization = paddle.log(2. * pi)
    inv_sigma = paddle.exp(-log_var)
    tmp = (samples - mean)
...
def total_correlation(z, z_mean, z_logvar):
    ...
    log_qz_prob = gaussian_log_density(
        paddle.unsqueeze(z, 1), paddle.unsqueeze(z_mean, 0),
        paddle.unsqueeze(z_logvar, 0))
    # Compute log prod_l p(z(x_j)_l) = sum_l(log(sum_i(q(z(z_j)_l|x_i)))
    # + constant) for each sample in the batch, which is a vector of size
    # [batch_size,].
    log_qz_product = paddle.sum(
        paddle.logsumexp(log_qz_prob, axis=1, keepdim=False),
        dim=1,
        keepdim=False)
    # Compute log(q(z(x_j))) as log(sum_i(q(z(x_j)|x_i))) + constant =
    # log(sum_i(prod_l q(z(x_j)_l|x_i))) + constant.
    log_qz = paddle.logsumexp(
        paddle.sum(log_qz_prob, axis=2, keepdim=False),
        dim=1,
        keepdim=False)
    return paddle.mean(log_qz - log_qz_product)
