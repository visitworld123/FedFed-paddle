...
class Data_Loader(object):
    ...
    def load_full_data(self):
        ...
        if self.dataset == "SVHN":
            ...
            train_ds = self.full_data_obj(self.datadir,  mode='train', download=True, transform=train_transform)
            test_ds = self.full_data_obj(self.datadir,  mode='test', download=True, transform=test_transform)
            logging.info(os.getcwd())

        train_ds.data, train_ds.targets = list(zip(*train_ds.data))
        # test_ori_ds.data, test_ori_ds.targets = list(zip(*test_ori_ds.data))

        train_ds.data = np.array(train_ds.data).reshape(-1,3,32,32)
        train_ds.data = train_ds.data.transpose([0,2,3,1])
...
# got it 加载不同子数据集
    ...
    def get_dataloader(self, train_ds, test_ds,shuffle=True, drop_last=False, train_sampler=None, num_workers=1):
        ...
        train_dl = data.DataLoader(dataset=train_ds, batch_size=self.batch_size, shuffle=shuffle,   # dl means dataloader
                                drop_last=drop_last, batch_sampler=train_sampler, num_workers=num_workers) # sampler定义自己的sampler策略，如果指定这个参数，则shuffle必须为False
