...
class FcNet(nn.Layer):
    ...
    def __init__(self, input_dim, hidden_dims, output_dim, dropout_p=0.0):
        ...
        for i in range(len(self.dims) - 1):
            ...
            self.layers.append(
                nn.Linear(ip_dim, op_dim, bias_attr=True)
            )
...
class PerceptronModel(nn.Layer):
...
class SimpleCNNContainer(nn.Layer):
...
### Moderate size of CNN for CIFAR-10 dataset
class ModerateCNNCeleba(nn.Layer):
    def __init__(self):
...
class ModerateCNNMNIST(nn.Layer):
...
class ModerateCNNContainer(nn.Layer):
